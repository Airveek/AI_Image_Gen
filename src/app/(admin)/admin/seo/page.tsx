...
export default async function AdminSeoPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      {data.setupMessage ? <p className="rounded-2xl border border-warning/30 bg-warning-soft p-5 text-sm leading-6 text-warning">{data.setupMessage}</p> : null}
      <SeoBriefIntakePanel />
      ...
      <SeoDashboardTabs
        tabs={[
          ...
          },
          {
            id: "coverage",
            label: "Coverage",
            panel: <section className="space-y-4" aria-labelledby="coverage-heading"><h2 id="coverage-heading" className="font-display text-xl font-bold text-brand-white">Coverage</h2><div className="grid gap-4 sm:grid-cols-3">{coverageCards.map(([label, value]) => <MetricCard key={label} label={label} value={value} />)}</div><p className="rounded-2xl border border-border bg-brand-panel p-5 text-sm leading-6 text-muted">Only live, canonical, indexable pages can enter a sitemap. Orphan, non-200, noindex, or failed pages remain out of the public discovery surface.</p></section>,
          },
          {
            id: "performance",
            label: "Performance",
            panel: <section className="space-y-4" aria-labelledby="performance-heading"><h2 id="performance-heading" className="font-display text-xl font-bold text-brand-white">Performance</h2><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{performanceCards.map(([label, value]) => <MetricCard key={label} label={label} value={value} />)}</div><div className="grid gap-4 sm:grid-cols-3">{vitalCards.map(([label, value]) => <MetricCard key={label} label={`P75 ${label}`} value={value} />)}</div><div className="grid gap-6 lg:grid-cols-2"><AttributionCard title="First-touch attribution" rows={data.attribution.firstTouch} /><AttributionCard title="Last non-direct attribution" rows={data.attribution.lastNonDirect} /></div><p className="rounded-2xl border border-border bg-brand-panel p-5 text-sm leading-6 text-muted">GSC is the pre-click source of truth; GA4/BigQuery measures anonymous behavior; Whop remains the payment and refund source of truth. Core Web Vitals are consent-gated and reported as anonymous 75th-percentile samples; targets are LCP ≤2.5s, INP ≤200ms, and CLS ≤0.1.</p></section>,
          },
          {
            id: "quality-links",
            label: "Quality & links",
            panel: <section className="grid gap-6 lg:grid-cols-2" aria-labelledby="quality-heading"><div className="rounded-2xl border border-border bg-brand-panel p-6"><h2 id="quality-heading" className="font-display text-xl font-bold text-brand-white">Quality & links</h2><p className="mt-3 text-sm leading-6 text-muted">Every page is checked for evidence, rights, structured content, schema, canonical state, CTA health, and crawlable inbound/outbound links before publication.</p><p className="mt-4 text-sm text-brand-soft">Open alerts: {data.summary.openAlerts} · Open recommendations: {data.summary.openRecommendations}</p></div><AlertList alerts={data.alerts} /><RecommendationList recommendations={data.recommendations} /></section>,
          },
          {
            id: "experiments",
            label: "Experiments",
            panel: <section className="rounded-2xl border border-border bg-brand-panel p-6" aria-labelledby="experiments-heading"><h2 id="experiments-heading" className="font-display text-xl font-bold text-brand-white">Experiments</h2><p className="mt-3 text-sm leading-6 text-muted">Automatic publication mode is active: template rollout statuses are retained for history but do not hold approved pages. Technical, content-quality, render, sitemap, refresh, merge, canonical, redirect, and pruning controls remain enforced.</p></section>,
          },
          {
            id: "operations",
            label: "Operations",
...
function OperationsSummary({ data, operationsQueue }: { data: AdminSeoDashboardData; operationsQueue: Awaited<ReturnType<typeof listSeoOperationsQueue>> }) {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-border bg-brand-panel p-6" aria-labelledby="operations-heading">
        <h2 id="operations-heading" className="font-display text-xl font-bold text-brand-white">Operations</h2>
        <div className="mt-4 grid gap-2 sm:grid-cols-4">{Object.entries(integrationReadiness).map(([label, ready]) => <div className="rounded-xl border border-border p-3" key={label}><p className="text-xs font-bold uppercase tracking-[0.12em] text-muted">{label}</p><p className={`mt-2 text-sm font-bold ${ready ? "text-brand-neon" : "text-amber-200"}`}>{ready ? "Ready" : "Setup needed"}</p></div>)}</div>
        <div className="mt-4 rounded-xl border border-border p-4"><div className="flex flex-wrap items-baseline justify-between gap-2"><p className="text-xs font-bold uppercase tracking-[0.12em] text-muted">Active content team</p><p className={`text-xs font-semibold ${membersReady ? "text-brand-neon" : "text-amber-200"}`}>{membersReady ? "Ready for controlled automation" : "Writer + publisher required"}</p></div><div className="mt-3 grid gap-2 sm:grid-cols-3"><MetricCard label="Active writers" value={data.operations.contentMembers.writers} /><MetricCard label="Active publishers" value={data.operations.contentMembers.publishers} /><MetricCard label="Active SEO admins" value={data.operations.contentMembers.seoAdmins} /></div></div>
        <div className="mt-4 rounded-xl border border-border p-4"><p className="text-xs font-bold uppercase tracking-[0.12em] text-muted">Measurement import watermarks</p><div className="mt-3 grid gap-2 sm:grid-cols-3">{data.importWatermarks.map((item) => <div className="rounded-xl border border-border p-3" key={item.source}><div className="flex items-center justify-between gap-2"><p className="text-sm font-bold uppercase text-brand-white">{item.source}</p><span className={`text-xs font-bold uppercase ${item.status === "failed" ? "text-amber-200" : item.status === "succeeded" ? "text-brand-neon" : "text-muted"}`}>{item.status}</span></div><p className="mt-2 text-xs text-muted">Success: {item.lastSuccessMetricDate ?? "—"}</p><p className="text-xs text-muted">Attempt: {item.lastAttemptedMetricDate ?? "—"}</p>{item.lastError ? <p className="mt-2 line-clamp-2 text-xs text-warning">{item.lastError}</p> : null}</div>)}</div></div>
        <div className="mt-4 rounded-xl border border-border p-4"><div className="flex flex-wrap items-baseline justify-between gap-2"><p className="text-xs font-bold uppercase tracking-[0.12em] text-muted">Keyword evidence feedback</p><p className="text-xs text-muted">Since {data.periodDays} days · latest {data.keywordEvidence.latestMetricDate ?? "—"}</p></div><div className="mt-3 grid gap-2 sm:grid-cols-4"><MetricCard label="Evidence rows" value={data.keywordEvidence.totalRows} /><MetricCard label="Measured rows" value={data.keywordEvidence.measuredRows} /><MetricCard label="Qualitative rows" value={data.keywordEvidence.qualitativeRows} /><MetricCard label="Brief/page-linked" value={data.keywordEvidence.linkedRows} /></div>{data.keywordEvidence.sources.length ? <div className="mt-3 flex flex-wrap gap-2">{data.keywordEvidence.sources.map((item) => <span className="rounded-full border border-border px-3 py-1 text-xs text-brand-soft" key={item.source}>{item.source}: {item.rows}</span>)}</div> : <p className="mt-3 text-xs text-muted">No keyword evidence has been imported yet. The ingest command is dry-run by default and never publishes.</p>}</div>
...
function StatusChips({ title, values }: { title: string; values: Record<string, number> }) {
  ...
  return <div className="mt-4 rounded-xl border border-border p-4"><p className="text-xs font-bold uppercase tracking-[0.12em] text-muted">{title}</p><div className="mt-3 flex flex-wrap gap-2">{Object.entries(values).map(([status, count]) => <span className="rounded-full border border-border px-3 py-1 text-xs text-brand-soft" key={status}>{status}: {count}</span>)}</div></div>;
}

function JobRuns({ jobs }: { jobs: AdminSeoJobRun[] }) {
  return jobs.length ? <ul className="mt-4 grid gap-3 sm:grid-cols-2">{jobs.map((job) => <li className="flex items-center justify-between gap-4 rounded-xl border border-border p-4" key={job.id}><div><p className="font-semibold text-brand-white">{job.loopName}</p><p className="mt-1 text-xs text-muted">Checked {job.checkedCount} · acted {job.actedCount}</p></div><span className="text-xs font-bold uppercase text-brand-neon">{job.status}</span></li>)}</ul> : <p className="mt-4 rounded-xl border border-dashed border-border p-6 text-center text-sm text-muted">SEO jobs will appear after the Inngest control plane is enabled.</p>;
}

function MetricCard({ label, value }: { label: string; value: number | string }) {
  return <div className="rounded-2xl border border-border bg-brand-panel p-5"><p className="text-sm text-muted">{label}</p><p className="mt-3 font-display text-3xl font-bold text-brand-white">{value}</p></div>;
}

function AlertList({ alerts }: { alerts: Array<{ id: string; severity: string; category: string; title: string; message: string }> }) {
  return alerts.length ? <div className="rounded-2xl border border-border bg-brand-panel p-6"><h2 className="font-display text-xl font-bold text-brand-white">Current blockers</h2><ul className="mt-4 space-y-3">{alerts.slice(0, 8).map((alert) => <li className="rounded-xl border border-border p-4" key={alert.id}><p className="text-xs font-bold uppercase text-brand-neon">{alert.severity} · {alert.category}</p><p className="mt-1 font-semibold text-brand-white">{alert.title}</p><p className="mt-1 text-sm text-muted">{alert.message}</p></li>)}</ul></div> : <div className="rounded-2xl border border-border bg-brand-panel p-6"><h2 className="font-display text-xl font-bold text-brand-white">Current blockers</h2><p className="mt-4 rounded-xl border border-dashed border-border p-6 text-center text-sm text-muted">No SEO alerts recorded.</p></div>;
}
...
function RecommendationList({ recommendations }: { recommendations: Array<{ id: string; severity: string; category: string; title: string; message: string; recommendedAction: string; query: string | null; status: SeoRecommendationStatus }> }) {
  return recommendations.length ? <div className="rounded-2xl border border-border bg-brand-panel p-6 lg:col-span-2"><div className="flex flex-wrap items-baseline justify-between gap-3"><h2 className="font-display text-xl font-bold text-brand-white">Improvement queue</h2><span className="text-xs text-muted">deduplicated measurement work</span></div><ul className="mt-4 grid gap-3 lg:grid-cols-2">{recommendations.slice(0, 12).map((recommendation) => <li className="rounded-xl border border-border p-4" key={recommendation.id}><p className="text-xs font-bold uppercase text-brand-neon">{recommendation.severity} · {recommendation.category}{recommendation.query ? ` · ${recommendation.query}` : ""}</p><p className="mt-1 font-semibold text-brand-white">{recommendation.title}</p><p className="mt-1 text-sm text-muted">{recommendation.message}</p><p className="mt-3 text-sm leading-6 text-brand-soft"><span className="font-semibold text-brand-white">Next action:</span> {recommendation.recommendedAction}</p><SeoRecommendationActions currentStatus={recommendation.status} recommendationId={recommendation.id} /></li>)}</ul></div> : <div className="rounded-2xl border border-border bg-brand-panel p-6 lg:col-span-2"><h2 className="font-display text-xl font-bold text-brand-white">Improvement queue</h2><p className="mt-4 rounded-xl border border-dashed border-border p-6 text-center text-sm text-muted">No measurement recommendations recorded.</p></div>;
}
