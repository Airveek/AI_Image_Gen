...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const page = await getLiveSeoPage(`/authors/${(await params).slug}`);
  if (!page) return { title: "Airveek author" };
...
export default async function AuthorPage({ params }: Props) {
  const page = await getLiveSeoPage(`/authors/${(await params).slug}`);
  if (!page) notFound();
  return <InteriorPageShell><SeoContentPage page={page} breadcrumbs={[{ label: "Home", href: "/" }, { label: "Authors", href: "/authors" }, { label: page.title }]} /></InteriorPageShell>;
}
