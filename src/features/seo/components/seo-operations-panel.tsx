  ...
  return (
    <Card className="mt-6 overflow-hidden">
      <div className="border-b border-border p-5 sm:p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold"><span className={`rounded-full border px-3 py-1 ${pendingRights ? "border-amber-200/30 bg-amber-200/10 text-warning" : "border-brand-neon/30 bg-brand-neon/10 text-brand-neon"}`}>Human rights approvals pending: {pendingRights}</span><span className="rounded-full border border-border px-3 py-1 text-muted">Rights approval never publishes a page</span></div>
      </div>

      {feedback ? <p aria-live="polite" className={`mx-5 mt-5 rounded-xl border px-4 py-3 text-sm sm:mx-6 ${hasError ? "border-red-300/20 bg-red-300/5 text-danger" : "border-brand-neon/20 bg-brand-neon/5 text-brand-soft"}`} role="status">{feedback}</p> : null}

      <div className="grid gap-6 p-5 sm:p-6 xl:grid-cols-2">
        <form action={assignmentAction} className="space-y-4 rounded-2xl border border-border bg-surface-muted p-5">
          ...
          {briefs.length === 0 ? <p className="text-xs text-warning">No open briefs are currently available.</p> : null}
          {members.length === 0 ? <p className="text-xs text-warning">No active content members are configured.</p> : null}
        </form>

        <form action={reviewAction} className="space-y-4 rounded-2xl border border-border bg-surface-muted p-5">
      ...
      <div className="border-t border-border p-5 sm:p-6">
        <form action={rightsAction} className="space-y-4 rounded-2xl border border-amber-200/20 bg-amber-200/[0.025] p-5">
          ...
          {briefs.length === 0 ? <p className="text-xs text-warning">Create a brief before recording rights evidence.</p> : null}
        </form>
      </div>
      ...
      <div className="border-t border-border p-5 sm:p-6">
        ...
        {rollouts.length ? <div className="mt-4 space-y-4">{rollouts.map((rollout) => <form action={rolloutAction} className="grid gap-4 rounded-2xl border border-border bg-surface-muted p-5 lg:grid-cols-[1.2fr_1fr_1fr_1fr_auto] lg:items-end" key={rollout.templateVersion}>
        ...
        </form>)}</div> : <p className="mt-4 rounded-xl border border-dashed border-border p-5 text-sm text-muted">No template rollout rows exist yet. New page templates will be registered automatically in manual review.</p>}
      </div>

      <div className="border-t border-border p-5 sm:p-6">
        <h4 className="font-display text-lg font-bold text-brand-white">Open brief queue</h4>
        {briefs.length ? <div className="mt-4 space-y-3">{briefs.map((brief) => <div className="rounded-xl border border-border bg-media-stage p-4" key={brief.id}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold text-brand-white">{brief.briefKey}</p><p className="mt-1 text-sm text-muted">{brief.productEntity} · {brief.pageFamily} · {brief.primaryQuery}</p></div><div className="flex flex-wrap gap-2"><span className="rounded-full border border-border px-3 py-1 text-xs font-bold uppercase text-brand-soft">{brief.status}</span><span className={`rounded-full border px-3 py-1 text-xs font-bold uppercase ${brief.rightsStatus === "approved" ? "border-brand-neon/30 text-brand-neon" : "border-amber-200/30 text-warning"}`}>rights {brief.rightsStatus}</span></div></div><div className="mt-3 flex flex-wrap gap-2 text-xs text-muted"><span>Priority {brief.priority}</span><span>•</span><span>{brief.pageId ? "Draft linked" : "Awaiting draft"}</span>{(assignmentByBrief.get(brief.id) ?? []).map((assignment) => <span className="rounded-full bg-surface-raised px-2 py-1" key={assignment.id}>{assignment.assignmentRole}</span>)}</div>{brief.rightsStatus !== "approved" ? <div className="mt-3 rounded-lg border border-amber-200/10 bg-amber-200/[0.03] p-3 text-xs leading-5 text-warning"><p>Source asset: {brief.sourceAssetPath ?? "missing path"}</p><p>Checksum: {brief.sourceAssetChecksum ?? "compute and verify locally before approval"}</p></div> : null}</div>)}</div> : <p className="mt-4 rounded-xl border border-dashed border-border p-6 text-center text-sm text-muted">The queue is empty. Create a reviewed brief before assigning work.</p>}
      </div>
    </Card>
  );
}
