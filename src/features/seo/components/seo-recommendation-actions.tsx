    ...
    <div className="mt-4 border-t border-white/10 pt-3">
      <form action={action} className="grid gap-2 sm:grid-cols-[minmax(0,12rem)_minmax(0,1fr)_auto] sm:items-end">
        ...
        <label className="block space-y-1 text-xs font-semibold text-brand-soft">
          ...
          <select className="min-h-10 w-full rounded-lg border border-white/10 bg-brand-black px-2 text-xs text-brand-white" name="status" onChange={(event) => setNextStatus(event.target.value as SeoRecommendationStatus)} value={nextStatus}>
        ...
        <label className="block space-y-1 text-xs font-semibold text-brand-soft">
          ...
          <textarea className="min-h-10 w-full rounded-lg border border-white/10 bg-brand-black px-2 py-2 text-xs text-brand-white placeholder:text-muted" maxLength={4000} name="resolutionNote" placeholder={requiresNote ? "What evidence-backed action was taken?" : "Optional context for the next operator."} required={requiresNote} rows={2} />
        </label>
      ...
      {state.message ? <p aria-live="polite" className={`mt-2 text-xs ${state.status === "error" ? "text-red-100" : "text-brand-soft"}`} role="status">{state.message}</p> : null}
    </div>
  );
}
