...
export function CreatorBatchProgress({
  ...
  return (
    <section className="relative w-full max-w-5xl" data-testid="generation-batch-results" aria-busy={isGenerating}>
      ...
      <div className="mb-5 h-1.5 overflow-hidden rounded-full bg-surface-raised" role="progressbar" aria-label="Image generation progress" aria-valuemin={0} aria-valuemax={items.length} aria-valuenow={readyCount}>
...
function BatchResultCard({ item, onRetry, retryDisabled, onOpenImage }: {
  ...
  return (
    <article className="overflow-hidden rounded-xl border border-border bg-surface-raised" data-testid={`generation-item-${item.index}`}>
      <div className="relative aspect-square bg-media-stage">
        {readyImage ? (
          <>
            <button type="button" className="group absolute inset-0 block h-full w-full cursor-zoom-in focus-visible:outline focus-visible:outline-2 focus-visible:outline-inset focus-visible:outline-focus" onClick={() => onOpenImage(readyImage.asset)} aria-label={`Open Image ${item.index}`}>
          ...
          <div className="flex h-full flex-col items-center justify-center gap-2 px-5 text-center text-danger"><CircleAlert className="h-7 w-7 text-danger" aria-hidden="true" /><span>Image {item.index} could not be created.</span></div>
        )}
      </div>
      <div className="flex items-center justify-between gap-3 p-3">
        <div className="min-w-0"><h3 className="text-sm font-semibold text-foreground">Image {item.index}</h3>{readyImage ? <p className="mt-1 text-xs text-brand-soft">Saved to your library</p> : item.error ? <p className="mt-1 line-clamp-2 text-xs text-danger">{item.error}</p> : null}</div>
