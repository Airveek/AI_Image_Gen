...
import { ArrowDownAZ, ArrowDownUp, Check, ChevronRight, LoaderCircle, Plus, Search, X } from "lucide-react";
...
export function CreatorAssetPicker({
  ...
  const [openGroup, setOpenGroup] = useState<AssetGroupId>("products");
    ...
    return (
      <div ref={pickerRef} className={cn("h-full min-h-0 overflow-y-auto overscroll-contain", compact ? "p-3" : "px-3 pb-4 pt-3")}>
        <label className="relative block" htmlFor={searchId}>
          ...
          <input id={searchId} type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search assets" className="min-h-10 w-full rounded-xl border border-white/12 bg-white/[0.045] pl-10 pr-9 text-sm text-white placeholder:text-brand-gray focus:border-brand-neon/50 focus:outline-none" />
          {search ? <button type="button" onClick={() => setSearch("")} className="absolute right-1 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-lg text-muted hover:bg-white/[0.08] hover:text-white" aria-label="Clear search"><X className="h-4 w-4" aria-hidden="true" /></button> : null}
        </label>
            ...
            return (
              <section key={group.id} aria-label={group.label} className="border-b border-white/[0.06] pb-1 last:border-0">
                <div className="flex min-h-10 items-center gap-1">
                  <button type="button" onClick={() => { setOpenGroup(expanded ? "products" : group.id); setSortGroup(null); }} aria-expanded={expanded} className="flex min-h-10 min-w-0 flex-1 items-center gap-2 rounded-lg px-1.5 text-left text-sm font-medium text-white hover:bg-white/[0.045] focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-neon"><ChevronRight className={cn("h-3.5 w-3.5 shrink-0 text-muted transition-transform", expanded && "rotate-90")} aria-hidden="true" /><span className="truncate">{group.label}</span></button>
                  <div className="relative flex shrink-0 items-center">
                    <button type="button" onClick={() => setSortGroup(sortOpen ? null : group.id)} className="flex h-9 w-9 items-center justify-center rounded-lg text-muted hover:bg-white/[0.06] hover:text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-neon" aria-label={`Sort ${group.label}`} aria-haspopup="menu" aria-expanded={sortOpen}><ArrowDownUp className="h-3.5 w-3.5" aria-hidden="true" /></button>
                    {group.newLabel ? <button type="button" onClick={() => openUpload(group.role)} className="flex h-9 w-9 items-center justify-center rounded-lg text-muted hover:bg-white/[0.06] hover:text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-neon" aria-label={group.newLabel} title={group.newLabel} disabled={isUploading}><Plus className="h-4 w-4" aria-hidden="true" /></button> : null}
                    {sortOpen ? <div role="menu" aria-label={`Sort ${group.label}`} className="absolute right-0 top-10 z-30 min-w-44 rounded-xl border border-white/12 bg-[#202220] p-1.5 shadow-2xl"><button type="button" role="menuitemradio" aria-checked={sortModes[group.id] === "recent"} onClick={() => selectSort(group.id, "recent")} className="flex min-h-10 w-full items-center justify-between gap-3 rounded-lg px-3 text-left text-sm hover:bg-white/[0.08] focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-neon"><span>Recently added</span>{sortModes[group.id] === "recent" ? <Check className="h-4 w-4 text-brand-neon" aria-hidden="true" /> : null}</button><button type="button" role="menuitemradio" aria-checked={sortModes[group.id] === "az"} onClick={() => selectSort(group.id, "az")} className="flex min-h-10 w-full items-center justify-between gap-3 rounded-lg px-3 text-left text-sm hover:bg-white/[0.08] focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-neon"><span>A–Z</span>{sortModes[group.id] === "az" ? <ArrowDownAZ className="h-4 w-4 text-brand-neon" aria-hidden="true" /> : null}</button></div> : null}
                  </div>
                </div>
                {expanded ? <div className="grid grid-cols-2 gap-2 px-1 pb-2 pt-1">{group.assets.slice(0, compact ? 12 : 40).map((asset) => <AssetCard key={asset.id} asset={asset} role={group.role} references={references} onToggle={onToggle} expanded />)}{group.assets.length === 0 ? <p className="col-span-2 px-2 py-4 text-xs leading-5 text-muted">{normalizedSearch ? "No matching assets." : "No saved assets yet."}</p> : null}</div> : <div className="flex gap-2 overflow-hidden px-1 pb-2 pt-1">{group.assets.slice(0, 4).map((asset) => <AssetCard key={asset.id} asset={asset} role={group.role} references={references} onToggle={onToggle} />)}{group.assets.length === 0 ? <p className="px-2 py-2 text-xs text-muted">No assets</p> : null}</div>}
              </section>
            );
          })}
        </div>
        {groups.length === 0 ? <div className="mt-4 rounded-xl border border-dashed border-white/12 p-5 text-center text-xs leading-5 text-muted">No matching assets. Try another search.</div> : null}
  ...
  return (
    <div className={cn(compact ? "p-3" : "p-4", !compact && "h-full overflow-y-auto overscroll-contain")}>
      ...
      <label className={cn("relative block", compact ? "mt-3" : "mt-4")} htmlFor={searchId}><span className="sr-only">Search assets</span><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted" aria-hidden="true" /><input id={searchId} type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search assets" className={cn("w-full rounded-xl border border-white/12 bg-white/[0.045] pl-10 pr-3 text-sm text-white placeholder:text-brand-gray focus:border-brand-neon/50 focus:outline-none", compact ? "min-h-10" : "min-h-11")} /></label>
      <input ref={fileInputRef} className="sr-only" type="file" accept="image/png,image/jpeg,image/webp" data-testid={uploadInputTestId} onChange={(event) => { const file = event.target.files?.[0]; const role = preferredRole ?? pendingUploadRole; if (file) void onUpload(file, assetKindForRole(role), role); event.target.value = ""; }} />
      <div className={cn("space-y-5", compact ? "mt-3 space-y-3" : "mt-5")}>{classicGroups.map((group) => <section key={group.id} aria-label={group.label}><h3 className="mb-1.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-muted">{group.label}</h3><div className="grid grid-cols-4 gap-2">{group.assets.slice(0, group.kind === "generation" ? 12 : 16).map((asset) => <AssetCard key={asset.id} asset={asset} role={group.role} references={references} onToggle={onToggle} />)}</div></section>)}{readyCount === 0 ? <div className="rounded-xl border border-dashed border-white/12 p-5 text-center text-xs leading-5 text-muted">No saved assets yet. Upload the first image above.</div> : groups.length === 0 ? <div className="rounded-xl border border-dashed border-white/12 p-5 text-center text-xs leading-5 text-muted">No matching assets. Try another search or upload a new image.</div> : null}</div>
    </div>
  );
}

function AssetCard({ asset, role, references, onToggle, expanded = false }: { asset: CreatorAsset; role: ReferenceRole; references: GenerationReference[]; onToggle: (asset: CreatorAsset, role: ReferenceRole) => void; expanded?: boolean }) {
  const selectedIndex = references.findIndex((reference) => reference.assetId === asset.id);
  const selected = selectedIndex >= 0;
  return <button type="button" onClick={() => onToggle(asset, role)} aria-pressed={selected} aria-label={`${selected ? "Remove" : "Use"} ${asset.name} as ${referenceRoleLabel(role)}`} title={asset.name} className={cn("group relative min-w-0 overflow-hidden rounded-lg border bg-brand-panel text-left transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-neon", expanded ? "aspect-[1/1.05]" : "h-16 w-16 shrink-0", selected ? "border-brand-neon ring-2 ring-brand-neon/20" : "border-white/10 hover:border-white/30")}>{asset.imageUrl ? <Image src={asset.imageUrl} alt="" fill unoptimized className="object-cover" sizes={expanded ? "120px" : "64px"} /> : null}{expanded ? <span className="absolute inset-x-0 bottom-0 truncate bg-gradient-to-t from-black/95 to-transparent px-2 pb-1.5 pt-5 text-[11px] text-white">{asset.name}</span> : null}{selected ? <span className="absolute right-1 top-1 flex h-6 w-6 items-center justify-center rounded-full bg-brand-neon text-xs font-bold text-black"><Check className="h-3.5 w-3.5" aria-hidden="true" /></span> : null}</button>;
}
