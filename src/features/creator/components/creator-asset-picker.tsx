...
function AssetCard({ asset, role, references, onToggle, expanded = false }: { asset: CreatorAsset; role: ReferenceRole; references: GenerationReference[]; onToggle: (asset: CreatorAsset, role: ReferenceRole) => void; expanded?: boolean }) {
  ...
  return <button type="button" onClick={() => onToggle(asset, role)} aria-pressed={selected} aria-label={`${selected ? "Remove" : "Use"} ${asset.name} as ${referenceRoleLabel(role)}`} title={asset.name} className={cn("group relative min-w-0 overflow-hidden rounded-lg border bg-brand-panel text-left transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-focus", expanded ? "aspect-[1/1.05]" : "h-16 w-16 shrink-0", selected ? "border-brand-neon ring-2 ring-brand-neon/20" : "border-border hover:border-primary/30")}>{asset.imageUrl ? <Image src={asset.imageUrl} alt="" fill unoptimized className="object-cover" sizes={expanded ? "120px" : "64px"} /> : null}{expanded ? <span className="absolute inset-x-0 bottom-0 truncate bg-gradient-to-t from-black/95 to-transparent px-2 pb-1.5 pt-5 text-[11px] text-white">{asset.name}</span> : null}{selected ? <span className="absolute right-1 top-1 flex h-6 w-6 items-center justify-center rounded-full bg-brand-neon text-xs font-bold text-primary-foreground"><Check className="h-3.5 w-3.5" aria-hidden="true" /></span> : null}</button>;
}
