...
export function buildSeoAttributionCookieMutation(
  input: BuildSeoAttributionInput,
): SeoAttributionCookieMutation {
  if (input.consentState === "denied") {
    return input.existingCookieValue ? { action: "clear" } : { action: "none" };
  }

  if (input.consentState !== "granted" || !isStrongSigningSecret(input.signingSecret)) {
    return { action: "none" };
  }

  const existing = input.existingCookieValue
    ? parseSeoAttributionCookie(input.existingCookieValue, input.signingSecret)
    : null;
  ...
  const lastNonDirectTouch = touch && touch.medium !== "direct"
    ? touch
    : existing?.lastNonDirectTouch ?? null;
...
export function parseSeoAttributionCookie(
  value: string,
  signingSecret: string,
): SeoAttributionCookie | null {
...
export function getSeoAttributionSigningSecret(): string | null {
  ...
  return isStrongSigningSecret(secret) ? secret : null;
}
...
function readTouch(currentUrl: string, referrer: string | null, siteHostname: string, now: Date): SeoTouch | null {
  ...
  if (!sourceParam && !mediumParam && !campaign && !externalReferrer) {
    return null;
  }
...
function directTouch(currentUrl: string, now: Date): SeoTouch {
  ...
  try {
    ...
    // A malformed URL is represented as a direct visit to the site root.
  }
...
function classifyTouch(
  ...
  if (sourceParam || mediumParam) {
    return {
      ...
      medium: mediumParam ?? "unknown",
    };
  }

  if (referrerHost && SEARCH_HOSTS.some((host) => hostMatches(referrerHost, host))) {
    return { source: sourceName(referrerHost), medium: "organic" };
  }

  if (referrerHost && AI_REFERRAL_HOSTS.some((host) => hostMatches(referrerHost, host))) {
    return { source: sourceName(referrerHost), medium: "referral" };
  }
...
function isStrongSigningSecret(value: string | null | undefined): value is string {
...
function isSeoAttributionCookie(value: unknown): value is SeoAttributionCookie {
  ...
  return isSeoTouch(value.firstTouch)
    && (value.lastNonDirectTouch === null || isSeoTouch(value.lastNonDirectTouch))
...
function isSeoTouch(value: unknown): value is SeoTouch {
  ...
  return isUuid(value.id)
    ...
    && value.landingPath.startsWith("/")
    && typeof value.source === "string"
    && typeof value.medium === "string"
    && (value.campaign === null || typeof value.campaign === "string")
    && (value.referrerHost === null || typeof value.referrerHost === "string")
    && (value.contentId === null || typeof value.contentId === "string");
}
