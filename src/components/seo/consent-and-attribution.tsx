...
export function ConsentAndAttribution() {
  ...
  return <>
    ...
    {visible && !privateRoute ? <aside className="pointer-events-none fixed inset-x-4 bottom-4 z-50 mx-auto flex max-w-3xl flex-col gap-4 rounded-2xl border border-[#83ff00]/30 bg-[#071007]/95 p-5 text-sm text-[#d9ffb8] shadow-2xl backdrop-blur sm:bottom-auto sm:top-4 sm:flex-row sm:items-center sm:justify-between" role="dialog" aria-label="Analytics preferences"><p className="max-w-xl leading-6">Airveek uses optional analytics to understand which guides help people create and buy. Essential cookies work without this choice.</p><div className="pointer-events-auto flex shrink-0 gap-2"><button className="rounded-full border border-white/20 px-4 py-2 font-bold text-[#b8c5b2] hover:border-white/50" onClick={() => decide("denied")} type="button">Decline</button><button className="rounded-full bg-[#83ff00] px-4 py-2 font-black text-[#040404] hover:bg-[#b8ff6b]" onClick={() => decide("granted")} type="button">Allow analytics</button></div></aside> : null}
  </>;
}
