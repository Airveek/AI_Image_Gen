...
export function HomeTrustRow() {
  const items = ["One-time payment", "Commercial-use plans", "HD downloads", "30-day guarantee"];
  ...
  return (
    <article className={`relative rounded-[2rem] border p-6 shadow-sm sm:p-8 ${featured ? "border-primary/60 bg-primary/6" : "border-border bg-surface"}`}>
      ...
      <div className="mt-5 flex items-end gap-2">
        ...
        <span className="pb-1 text-sm font-semibold text-muted-foreground">one time</span>
      </div>
...
export function HomePricingAndGuarantee() {
  ...
  const premium = ["Everything in Commercial", "Consistent characters", "Your face in AI images", "Product mockups and virtual models", "Perfect text in AI images", "Faster generation and premium tools"];
  ...
  return (
    <section id="pricing" className="border-t border-border bg-surface-muted px-4 py-20 sm:px-6 lg:py-32" aria-labelledby="pricing-title">
      <div className="mx-auto max-w-7xl">
        <SectionHeading titleId="pricing-title" eyebrow="One-time pricing" title="Create without another monthly bill." description="Pay once, choose the capability level your business needs, and keep creating without a recurring software charge." size="display" />
        <div className="mx-auto mt-10 grid max-w-5xl gap-5 lg:grid-cols-2">
          <PricingCard title="Commercial" price="$49" description="The practical toolkit for everyday business images, campaigns, and client work." bestFor="you need dependable marketing visuals and commercial-use downloads." features={commercial} planKey="commercial" featured />
          <PricingCard title="Premium" price="$147" description="Advanced creation for businesses that need consistency, better text, and premium product workflows." bestFor="your brand depends on repeatable characters, readable text, mockups, or virtual models." features={premium} planKey="premium" />
        </div>
        <p className="mt-7 text-center text-xs font-semibold text-muted-foreground">Use coupon <span className="font-bold text-primary">SECRET10</span> for 10% off · No hidden fees · No monthly charges</p>
        <div className="mx-auto mt-10 flex max-w-5xl flex-col items-center gap-5 rounded-[2rem] border border-primary/25 bg-primary/6 p-7 text-center sm:flex-row sm:p-9 sm:text-left">
          ...
          <div className="flex shrink-0 flex-col gap-2 text-xs font-bold text-foreground">
            <span className="inline-flex items-center gap-2"><Clock3 className="size-4 text-primary" aria-hidden="true" /> One-time payment</span>
  ...
  return (
    <section className="signature-gradient relative isolate flex min-h-[520px] items-center justify-center overflow-hidden px-4 py-20 text-white sm:px-6 lg:min-h-[620px]">
      ...
      <div className="relative mx-auto flex max-w-5xl flex-col items-center text-center">
        ...
        <CtaButton className="mt-8 min-w-56" href="#pricing" variant="inverse" showArrow={false}>Get lifetime access from $49</CtaButton>
      </div>
    </section>
  );
}
