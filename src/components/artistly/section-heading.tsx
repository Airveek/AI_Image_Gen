...
export function SectionHeading({ eyebrow, title, description, align = "center", titleId, children }: SectionHeadingProps) {
  ...
  return (
    <div className={`max-w-3xl ${alignment} ${align === "center" ? "mx-auto" : ""}`}>
      {eyebrow ? <p className="mb-4 text-xs font-bold uppercase tracking-[0.24em] text-cyan-300">{eyebrow}</p> : null}
      <h2 id={titleId} className="font-display text-3xl font-extrabold leading-tight text-white sm:text-4xl lg:text-5xl">{title}</h2>
