...
-- Re-run semantic intent protection when a page moves into an approval or
-- publishing state. The original guard watched identity/embedding fields,
-- which left a status-only update as a possible bypass.
...
begin
  -- A reviewer may explicitly resolve a merge review, but only with a
  -- durable explanation and an unchanged page identity/embedding. This is a
  -- human decision, not an automatic similarity override.
  if tg_op = 'UPDATE'
  ...
  -- Avoid re-evaluating an unchanged page on unrelated edits. Status and
  -- collision-resolution fields are intentionally included so transitions
  -- into approval/live states cannot skip the guard.
  if tg_op = 'UPDATE'
     and new.locale is not distinct from old.locale
     and new.product_slug is not distinct from old.product_slug
     and new.normalized_intent_key is not distinct from old.normalized_intent_key
...
comment on function public.guard_seo_intent_collision() is
  'Rechecks semantic intent on approval/live transitions; explicit reviewer-resolved merge notes are the only human override.';
