...
-- Review decisions, brief state, and draft-page state form one workflow
-- transition. Keep them in one transaction so an editorial decision cannot be
-- recorded while the queue remains in its previous state after a transient
-- database failure.
