...
-- Expire and reconcile an agent handoff atomically. This prevents a recovery
-- retry from leaving a run expired while its brief, assignment, and audit log
...
begin
  ...
  recovery_status := case when p_requeue and p_expected_status = 'sent' then 'requeued' else 'blocked' end;
...
comment on function public.recover_seo_agent_run(uuid, text, timestamptz, boolean) is
  'Atomically expires a stale SEO agent handoff and reconciles its brief, assignment, and audit event.';
